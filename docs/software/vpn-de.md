...
=== "Client-Verbindungsdatei"
    ...
    DietPi generiert während der Installation automatisch eindeutige 2048-Bit-Server- und Client-Schlüssel und platziert sie in einer einheitlichen Client-Konfigurationsdatei. Sie benötigen diese Datei, um sich von einem Client mit Ihrem OpenVPN-Server zu verbinden.
    ...
    Sie müssen die Datei „DietPi_OpenVPN_Client.ovpn“ in einem Texteditor öffnen, um die Zieldomäne/IP-Adresse zu ändern. Dies kann alles sein, von einer Website-Adresse, einem No-IP-Domänennamen oder einer IP-Adresse.
...
=== "Router-Setup"
    ...
    Sie müssen Ihren Router einrichten, um den externen Zugriff zu ermöglichen.
...
=== "Verbinden mit Ihrem OpenVPN-Server (Windows)"
    ...
    Methode 2 - GUI:
    Wenn Sie die OpenVPN-GUI verwenden möchten, müssen Sie „DietPi_OpenVPN_Client.ovpn“ in den OpenVPN-Konfigurationsspeicherort kopieren (z. B.: „C:\Programme\OpenVPN\config“).

=== "OpenVPN + Pi-Hole"

    Damit VPN-Clients auf Ihre lokale Pi-hole-Instanz zugreifen können, müssen Sie DNS-Anfragen von allen Netzwerkschnittstellen zulassen:
    `pihole -a -i lokal`
...
PiVPN ist ein Installations- und Verwaltungstool für OpenVPN und WireGuard. Es hat auch einen Befehl "pivpn", der eine einfache Erstellung zusätzlicher Benutzerprofile und -konfigurationen ermöglicht.
...
Bei der Installation mit `dietpi-software` können Sie wählen, ob Sie WireGuard als VPN-Server oder Client installieren möchten.

=== "Als VPN-Server installieren"

    #### Allgemein

    Sie werden aufgefordert, Ihre öffentliche IP/Domäne und den Port einzugeben, auf dem der VPN-Server erreichbar sein soll. Denken Sie daran, den Port (UDP) über NAT auf Ihrem Router zu öffnen/weiterzuleiten.
    Während der Installation wird automatisch auch eine Client-Konfigurationsdatei erstellt unter:
    `/etc/wireguard/wg0-client.conf`.

    Konfigurieren Sie die Client-Konfiguration nach Ihren Bedürfnissen, sie enthält einige informative Kommentare. Standardmäßig wird der gesamte Client-Netzwerkverkehr durch den VPN-Tunnel geleitet, einschließlich DNS-Anforderungen, die vom DNS-Resolver des Servers aufgelöst werden.
    Wenn Sie beispielsweise die Server-Pi-Hole-Instanz nur auf dem Client verwenden möchten, aber den gesamten anderen Datenverkehr außerhalb des VPN-Tunnels halten möchten, würden Sie die folgenden Werte bearbeiten:

    - „DNS = 192.168.0.100“.
    - `AllowedIPs = 192.168.0.100/32` (wobei die IP mit der lokalen IP Ihres DietPi-Servers übereinstimmen muss)

    Wenn Ihr Client ein anderer Linux-Computer ist, auf dem iptables installiert ist, können Sie die beiden Kill-Switch-Zeilen auskommentieren, damit der gesamte Netzwerkverkehr automatisch deaktiviert wird, wenn die VPN-Verbindung unterbrochen wird.
    Wenn Ihr Client ein Mobiltelefon mit installierter WireGuard-App ist, können Sie die Konfiguration einfach anwenden, indem Sie einen QR-Code auf das Serverterminal drucken über:
    `grep -v '^#' /etc/wireguard/wg0-client.conf | qrencode -t ansiutf8`.
    Damit VPN-Clients auf Ihre lokale Pi-hole-Instanz zugreifen können, müssen Sie DNS-Anfragen von allen Netzwerkschnittstellen zulassen: `pihole -a -i local`.
...
=== "Als VPN-Client installieren"
    ...
    Wenn Sie sich mit einem anderen DietPi-Rechner verbinden möchten, verwenden Sie die generierte `/etc/wireguard/wg0-client.conf` wie oben erwähnt.
    Wenn keine WireGuard (Auto-)Startanleitung enthalten ist, Sie diese aber benötigen, gehen Sie bitte wie folgt vor:
...
???+ Information "Kernel-Update"
    ...
    Das WireGuard-Kernelmodul muss jedes Mal neu erstellt werden, wenn der Kernel aktualisiert wird. Auf den meisten Geräten geschieht dies automatisch, wenn der Kernel (+Header) über das APT-Paket aktualisiert wird, was dann normalerweise den Modulneuaufbau auslöst.
    Wenn Sie den Kernel außerhalb von APT über „source build“ oder Befehle wie „rpi-update“ aktualisieren, stellen Sie sicher, dass auch passende Kernel-Header installiert sind, und erstellen Sie das WireGuard-Modul neu über: „dpkg-reconfigure wireguard-dkms“.
