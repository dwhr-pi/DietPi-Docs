...
Mit Cuberite können Sie einen einzigen, blitzschnellen Minecraft-Server erstellen, der die Leistungsvorteile von C++ (anstelle von Java) nutzt und über eine praktische Weboberfläche verfügt.
...
=== "Dienst neu starten"
    ...
    Sie können den Dienst neu starten, indem Sie Folgendes ausführen:
...
Mit MineOS können Sie über eine einfache Weboberfläche problemlos mehrere Minecraft-Server erstellen.
...
=== "Zugriff auf Webinterface"
    ...
    - URL: `https://<your.IP>:8443`
      Sie können die Zertifikats-"Warnung" getrost ignorieren, falls eine angezeigt wird.
    - Benutzername: „root“.
...
=== "Informationen"
    ...
    Nukkit betreibt standardmäßig einen einzelnen Server, der im LAN verfügbar ist.
...
Diese Installation ist möglich durch eine Zusammenarbeit mit Dimitris Panokostas (Amiberry) und Daniel Knight (DietPi).
...
=== "1st run setup"
    ...
    - **Kickstarts (Amiga BIOS/Bootsystem)**
      Amiga Kickstart ROM-Images sind erforderlich, um die Systeme auszuführen, die Sie emulieren möchten. Diese können aufgrund von Urheberrechtsbeschränkungen nicht gebündelt werden.
      Wenn Sie das Amiga Forever-Produkt besitzen, können Sie Kickstarts, für die Sie berechtigt sind, legal herunterladen und verwenden von: <https://www.amigaroms.com/>
      **Anmerkung:** *Kickstart 1.3 (A500-A2500-A3000-CDTV) wird dringend empfohlen, um mit den meisten Spielen zu funktionieren.*
      Kickstarts können in `/mnt/dietpi_userdata/amiberry/kickstarts` abgelegt werden.
    - **Disketten (Amiga `.adf`-Images)**
      Disketten-Images von Amiga haben die Dateierweiterung `.adf`.
      Sie benötigen mindestens ein ADF-Image, um Ihr Amiga-Erlebnis zu beginnen.
      Laden Sie Ihren ADF oder platzieren Sie ihn dort, wo Sie ihn haben möchten, z. B. erstellen und verwenden Sie:
...
=== "Starten"
    ...
    - Optional können Sie den Amiberry-Autostart aktivieren, um so schnell wie möglich direkt in die Amiga-Umgebung zu booten, mit der geringstmöglichen Störung durch Linux.
...
=== "Erstelle eine Amiga-Konfiguration"
    ...
    - Unter Amiga-Modell: Wählen Sie das Amiga-Modell aus, das Sie emulieren möchten (Beispiel A500)
...
Als nächstes müssen Sie den Emulator für das Kickstart- und Disketten-Image einrichten, das Sie verwenden möchten:
    ...
    - Wählen Sie das ROM aus, das Sie verwenden möchten.
...
=== "Optional: Stellen Sie die CPU-Geschwindigkeit auf die höchste ein (empfohlen)"

    Dadurch wird der Amiga so schnell wie möglich emuliert und sichergestellt, dass Sie die maximale FPS für Ihre SBC-Hardware erhalten.
...
=== "Häufig gestellte Fragen"
    ...
    #### Wie kann ich das Konfigurationsfenster öffnen, nachdem der Emulator gestartet wurde?
    ...
    Bei Verwendung einer Tastatur sind die standardmäßigen Joystick-Steuerelemente:
    ...
    Eine ***niedrigere Auflösung*** kann die Leistung bei den meisten Spielen verbessern. Aus dem Hauptmenü des Emulators:

    - Wählen Sie auf der linken Seite Anzeige
    - 640 x 256 ist eine hohe Auflösung
    - 320 x 256 ist eine niedrige Auflösung und sollte eine verbesserte Leistung bieten
    ...
    Die Diskettenlaufwerk-Emulation ist standardmäßig auf "800 %" eingestellt. Dies reduziert die Ladezeiten um bis zu 8x.
    Sie können dies auf 100 % senken, was die Kompatibilität erhöht:
    ...
    Spiele laufen mit verschiedenen Auflösungen über das Hauptmenü des Emulators:

    - Wählen Sie auf der linken Seite Anzeige
    - Ändern Sie den Höhenwert auf 200 oder 256
...
**Gut zu GEHEN!**
Wenn Sie fertig sind, wählen Sie **Start**, um den Emulator zu starten. Habe Spaß!
...
=== "Häufig gestellte Fragen"
    ...
    Bevor Sie beginnen, benötigen Sie die Originalspieldateien einer legalen Kopie und Installation von Descent.
    Stellen Sie sicher, dass Sie einen der Dateiserver von DietPi installiert haben.
...
Steam ist für x86 gemacht, aber es und viele Steam-Spiele mit Linux-Versionen können mit Hilfe von [Box86](#box86) auf ARM ausgeführt werden. Beachten Sie, dass 2D-Spiele am besten zu spielen sind und komplexere Spiele möglicherweise eine Übertaktung Ihres Geräts erfordern.
...
??? Warnung „Bekannte Probleme auf ARM“
    ...
    - Daher ist es nicht möglich, Spiele vom Client aus zu suchen/hinzuzufügen, was über den Website-Store (funktioniert nicht für kostenlose Spiele) oder einen anderen Client erfolgen muss.
...
PaperMC führt standardmäßig einen einzelnen Server aus, der im LAN verfügbar ist, aber über einen Port weitergeleitet werden kann, damit andere Personen eine Verbindung herstellen können.
...
=== "Standardserver/Abfrageport"
    ...
    - Der standardmäßige Server-/Abfrageport ist: **25565**
...
=== "Speicherverwaltung"
    ...
    Da PaperMC als Java-Anwendung läuft, hängt der Systemspeicherbedarf stark von der Java-Heap-Größe ab, die angepasst werden kann. Die absolute Mindestkopfgröße, die zum Ausführen von PaperMC erforderlich ist, beträgt 512 MiB, was zu ~850 MiB insgesamt verwendetem Systemspeicher führt. Wir empfehlen, ihn für die Produktionsnutzung auf mindestens 1 GiB (~1,35 GiB Gesamtspeichernutzung) oder höher zu erhöhen, je nach Anzahl der Benutzer und installierten Plugins.

    Gehen Sie wie folgt vor, um die Größe des Java-Heapspeichers zu ändern:

    1. Führen Sie `dietpi-services` von der Konsole aus.
    2. Wählen Sie `papermc`.
    3. Wählen Sie `Bearbeiten`.
    4. Entkommentieren Sie die Zeile, die mit `#ExecStart=` beginnt (entfernen Sie das führende `#`) und fügen Sie z. B. `-Xms1G -Xmx1G` direkt hinter dem `java`-Befehl hinzu, um eine Heap-Größe von 1 GiB anzuwenden. Zusätzlich müssen Sie oben ein `ExecStart=` hinzufügen, um den Befehl des ursprünglichen Dienstes zu deaktivieren, sodass es schließlich so aussieht:
    ...
    5. Verwenden Sie ++ctrl+o++ und ++enter++, um die Datei zu speichern, dann ++ctrl+x++, um zum Menü „dietpi-services“ zurückzukehren. Beim Beenden werden Sie gefragt, ob Sie den Dienst neu starten möchten, um die Änderungen zu übernehmen.

    !!! Warnung "Bewahren Sie Ihre Auslagerungsdatei nicht auf einer SD-Karte auf!"
        Wenn der verwendete Systemspeicher den physischen Arbeitsspeicher Ihres SBC übersteigt, empfehlen wir dringend, die Auslagerungsdatei **nicht** auf einer SD-Karte, sondern auf einem externen Laufwerk zu speichern, da SD-Karten normalerweise die regelmäßige Verwendung nicht überleben Auslagerungsdatei schreibt lange, was im schlimmsten Fall zu Serverabstürzen und Datenverlust führt.
...
Mit Box86 können Sie **i386** Linux-Programme (z. B. Spiele) auf **ARMv7**-Systemen ausführen. Dank [binfmt_misc](https://en.wikipedia.org/wiki/Binfmt_misc), das standardmäßig aktiviert ist, können Sie **i386**-Binärdateien wie jede andere ausführbare Datei ausführen und Box86 wird automatisch aufgerufen.

##Box64

Mit Box64 können Sie **x86_64** Linux-Programme (z. B. Spiele) auf **ARMv8**-Systemen ausführen. Dank [binfmt_misc](https://en.wikipedia.org/wiki/Binfmt_misc), das standardmäßig aktiviert ist, können Sie **x86_64**-Binärdateien wie jede andere ausführbare Datei ausführen und Box64 wird automatisch aufgerufen.
