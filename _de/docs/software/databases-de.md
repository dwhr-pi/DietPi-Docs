...
??? Information "Wie führe ich **DietPi-Software** aus und installiere **optimierte Software**?"
    ...
    Wählen Sie **Software-optimiert** und wählen Sie einen oder mehrere Artikel aus. Klicken Sie abschließend auf `Installieren`. DietPi führt alle notwendigen Schritte aus, um diese Softwareelemente zu installieren und zu starten.
...
Häufig verwendete Operationen (_wie: Verwaltung von Datenbanken, Tabellen, Spalten, Relationen, Indizes etc._) können über die Web-Benutzeroberfläche ausgeführt werden. Mit derselben Anwendung können Sie auch direkt beliebige SQL-Anweisungen ausführen.
...
Quelle: Teil der SQLite-Dokumentation, die vom Autor D. Richard Hipp als gemeinfrei freigegeben wurde. SVG-Konvertierung von Mike Toews. [Öffentlicher Bereich](https://commons.wikimedia.org/w/index.php?curid=11675072)
...
**Redis** gehört zur Familie der Datenbanken, die Schlüsselwertspeicher genannt werden. Das Wesentliche eines Schlüsselwertspeichers ist die Fähigkeit, einige Daten, die als Wert bezeichnet werden, in einem Schlüssel zu speichern. Diese Daten können später nur abgerufen werden, wenn wir den genauen Schlüssel kennen, mit dem sie gespeichert wurden.

=== "Schnellstart"

    Das erste, was Sie tun müssen, um zu überprüfen, ob Redis ordnungsgemäß funktioniert, ist das Senden eines PING-Befehls:
...
Die Daten können schön mit [**Grafana**](../hardware_projects/#grafana) betrachtet werden. Diese Installation und Dokumentation war möglich dank [@marcobrianza](https://github.com/MichaIng/DietPi/issues/1784#issuecomment-390778313).
...
=== "Gesicherten Zugriff aktivieren (HTTPS)"
    ...
    Standardmäßig ist die _HTTP_-Authentifizierung deaktiviert. Um es zu aktivieren, folgen Sie den nächsten zwei Schritten:
...
Der systemd-Dienst `postgresql.service` wird verwendet, um den PostgreSQL-Server zu starten und zu steuern. Folgende Befehle können verwendet werden:
...
=== "Protokolle anzeigen"
    ...
    Serviceprotokolle werden im Systemjournal erstellt und können angezeigt werden über:

    ```sh
    journalctl -u postgresql
    ```

    Der Server selbst protokolliert standardmäßig in einer Datei:
