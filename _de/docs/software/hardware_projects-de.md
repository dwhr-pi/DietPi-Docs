...
- [**Audiophonics PI-SPC - Leistungssteuerungsmodul für Raspberry Pi, ermöglicht das Ein-/Ausschalten über physische Tasten**](#audiophonics-pi-spc)
- [**Grafana - Die offene Plattform für schöne Analysen und Überwachung**](#grafana)
...
???+ Hinweis "Unsere Installation ist hochoptimiert und leicht"
    ...
    Wir empfehlen SSH dringend, um eine schnelle Einrichtung der Google-API und der Geräteverbindung zu ermöglichen.
...
=== "Erste Ausführung einrichten"
    ...
    - Sobald die Eingabeaufforderung "Ok, Google" erscheint, testen Sie das Gerät. Anschließend können Sie das Programm mit ++ctrl+c++ beenden und die Dienste neu starten:
...
=== "Ersteinrichtung"
    ...
    3. Sie sollten die Aufforderung sehen und bestenfalls hören, Ihr Gerät zu koppeln, zB: `PairingSkill - INFO - Kopplungscode: XXYYZZ`
...
=== "Mimik für Offline-TTS-Unterstützung (britische Männer) erstellen (optional)""
    ...
    Anmerkung: Dies erfordert ungefähr 3 GiB RAM und dauert eine Weile, stellen Sie also sicher, dass Sie über genügend Speicher verfügen (4 GiB empfohlen), falls Sie die Größe Ihrer Auslagerungsdatei erhöhen und sich einen Kaffee holen.
...
=== "Grundlegende Informationen"
    ...
    Unsere Standardinstallation enthält nicht die Desktop-Anwendung. Falls erforderlich, installieren Sie bitte zuerst einen Desktop und führen Sie dann den folgenden Befehl aus, um die GUI anschließend zu installieren:
...
Mit WebIOPi können Sie die GPIO-Hardware Ihres Raspberry Pi über eine Webschnittstelle steuern.
...
Um über das Internet auf Ihre WebIOPi-Oberfläche zugreifen zu können, können Sie [Remot3.it (Weaved)](../remote_desktop/#remot3it) installieren.
...
=== "Update auf aktuelle Version"
    ...
    Sie können Node-RED-Module über die Programmier-IDE aktualisieren. Um das Kernmodul „node-red“ zu aktualisieren, führen Sie den folgenden Befehl von der Konsole aus:
...
Mosquitto überwacht standardmäßig den Netzwerkport **1883**. Clients müssen sich mit den folgenden Anmeldeinformationen authentifizieren:
...
Führen Sie den folgenden Befehl aus, um das standardmäßige Authentifizierungspasswort für den Benutzer `dietpi` zu ändern:
...
`Mosquitto wird über sein offizielles APT-Repository installiert, daher können die folgenden Befehle verwendet werden, um es auf die neueste Version zu aktualisieren:
...
=== "Webinterface"
    ...
    - URL: `https://<your.IP>:9443/admin` (Sie können die Browserwarnung ignorieren, da standardmäßig ein selbstsigniertes Zertifikat verwendet wird.)
    - E-Mail-Adresse: `admin@blynk.cc`
    - Passwort: `<Ihr globales Passwort>` (Standard: `dietpi`)

=== "Einrichtungsdetails"

    DietPi installiert Blynk (einschließlich Benutzerdaten und Konfigurationsdatei) an folgendem Ort:

    ```
    /mnt/dietpi_userdata/blynk
    ```

    Protokolle können angezeigt werden über:
...
=== „Erste Schritte mit der Blynk-App“
    ...
    3. Dort können Sie die IP/Domäne Ihres eigenen Blynk-Servers eingeben und die oben genannten Anmeldeinformationen verwenden.
...
=== "Testskript ausführen"
    ...
    Sobald Sie ein Projekt in der iOS/Android-App erstellt haben, können Sie die Verbindung über `/mnt/dietpi_userdata/blynk/client-tcp-local.js` testen. Sie müssen die Datei bearbeiten und „YOUR_AUTH_TOKEN“ durch das Authentifizierungstoken Ihres Projekts ersetzen. Dann führen Sie es aus:

    ```sh
    /mnt/dietpi_userdata/blynk/client-tcp-local.js
    ```

    Innerhalb des Skripts können Sie auch Ereignis-Listener und -Handler definieren. Standardmäßig wird der Wert des virtuellen Pins „V1“ auf die Konsole gedruckt, wenn er sich ändert, und der Wert des virtuellen Pins „V9“ wird immer dann auf Systemzeitsekunden gesetzt, wenn der Pin gelesen wird. Für einen interaktiveren Test könnten Sie daher die folgenden Widgets per App zu Ihrem Projekt hinzufügen:
...
Leistungssteuerungsmodul für Raspberry Pi, mit dem Sie das System physisch ein- und ausschalten können, ohne „poweroff“ ausführen zu müssen.
...
Um das System auszuschalten, beginnen Sie mit dem Herunterfahren: Halten Sie den Netzschalter < 0,5 Sekunden lang gedrückt. Wenn Sie keinen Knopf haben, können Sie die Pins „BP PIN C“ und „BP PIN NO“ auch mit Ihrem bevorzugten elektrisch leitfähigen Metall (z. B. Pinzette) schließen.
...
Die Hauptschnittstelle zur Datenbank für die Verwaltung und die Datenübertragung sind HTTP-Anforderungen, die direkt vom Dienst „influxdb“ verarbeitet werden (der standardmäßig verwendete Port ist 8086).

Die Daten lassen sich schön mit Grafana betrachten.
Diese Installation und Dokumentation war möglich dank [@marcobrianza](https://github.com/MichaIng/DietPi/issues/1784#issuecomment-390778313).
...
Daten können durch Ausführen von:
...
Standardmäßig ist die HTTP-Authentifizierung deaktiviert. Um es zu aktivieren, ändern Sie `auth-enabled = true` in der Konfigurationsdatei `/etc/influxdb/influxdb.conf`. Starten Sie dann die Dienste mit `dietpi-services restart` neu.
...
Die offene Plattform für schöne Analysen und Überwachung.
...
=== "Voraussetzungen"
    ...
    Für Grafana ist ein Datenbankserver erforderlich. Da Grafana viele Optionen bietet (InfluxDB/MySQL), haben wir beides nicht automatisch installiert, da eine manuelle Konfiguration möglicherweise bevorzugt wird.
    Wir empfehlen jedoch dringend, [InfluxDB](../databases/#influxdb) zu installieren.

    Sie können dies mit den vom **DietPi-Software**-Tool bereitgestellten Installationsschritten tun oder die nächste Befehlszeile im Terminal ausführen:
...
=== "Nutzungsinformationen"
    ...
    Wenn Sie eine Datenbank gemäß der Online-Dokumentation von InfluxDB erstellt haben, befolgen Sie bitte diese Anweisungen:
