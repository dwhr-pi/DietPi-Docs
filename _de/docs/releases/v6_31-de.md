...
Willkommen zur Juli 2020-Veröffentlichung von DietPi. Es gibt eine Reihe von Updates in dieser Version, von denen wir hoffen, dass sie Ihnen gefallen werden.

### Verbesserungen

- **Raspberry Pi** - Unterstützung für Raspberry Pi OS (64-Bit) wurde hinzugefügt, einschließlich aller anderen Debian-Pre-Images auf RPi.
- **Odroid C4** - Unterstützung für diesen neuen Hardkernel SBC wurde hinzugefügt, um die Image-Erstellung basierend auf dem Meveric Odroid-Repository zu ermöglichen, einschließlich Kodi-Unterstützung.
- **Netzwerk "ping"** - es kann jetzt von allen Benutzern ohne Dateifähigkeiten, `sudo` oder `setuid` verwendet werden. Dafür erlauben wir allen Benutzern, native ICMP-Sockets zu erstellen, die seit Linux 3.X verfügbar sind, aber standardmäßig auf Debian deaktiviert sind. Andere Distributionen und systemd (Upstream) haben dies standardmäßig aktiviert und aus Sicherheits- und Benutzerfreundlichkeitsgründen folgen wir ihnen. Klicken Sie [hier](https://fedoraproject.org/wiki/Changes/EnableSysctlPingGroupRange) für weitere Details
...
- **DietPi-Cleaner** - Verbesserte Leistung des Dateibereinigers, wenn `/mnt` übersprungen wird, insbesondere in Kombination mit großen Laufwerken oder Netzwerkeinbindungen.
...
- **DietPi-Software - Firefox Sync Server** - Es wurde unserer Softwareliste hinzugefügt, die es ermöglicht, Ihre Firefox-Lesezeichen, Verlauf, Tabs und Passwörter über Ihren selbst gehosteten Server zu synchronisieren.
...
- **DietPi-Software - Samba Server** - Standardmäßige serverseitige Dateiberechtigungen von 0775 auf 0664 geändert, daher werden Dateien auf dem Server jetzt standardmäßig ohne Ausführungsberechtigungen erstellt, was den üblichen Standards entspricht.
...
- **System** - Mögliche Probleme aufgrund des fehlenden /media-Verzeichnisses auf DietPi-Images behoben, das standardmäßig nicht verwendet wird, aber Teil von FHS ist und daher von Linux-Software erwartet werden kann, z. B. `htpdate`.
...
- **DietPi-Config** - Behebung eines Problems bei RPi3+-Modellen, bei denen RPi3-Non-Plus-Übertaktungsprofile angezeigt wurden.
...
- **DietPi-Software - Sonarr/Radarr/Lidarr** - Behebung eines Problems, bei dem diese Softwaredienste einmal pro Stunde abstürzten, weil die Protokolldatei der SQLite-Datenbank fehlerhaft gelöscht wurde.
...
- **DietPi-Software - Ubooquity** - Das Installationsprogramm wurde aktualisiert, um die neueste Version direkt von Vae Mendis Software abzurufen, und die Berechtigungen wurden verbessert, um eine bessere Integration mit anderer Software zu ermöglichen.
...
- @Taloth von Sonarr: Vielen Dank, auch an alle anderen, die das Rätsel gemeldet, untersucht und schließlich gelöst haben: [Sonarr/Radarr-Dienste auf v6.29.2 fehlgeschlagen](https://dietpi.com/phpbb/viewtopic.php?t =7598)
